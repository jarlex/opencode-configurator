...
func (h HelpOverlayModel) View(background string) string {
	...
	bindings := []struct {
		...
		{h.keys.ToggleHidden.Help().Key, h.keys.ToggleHidden.Help().Desc},
		{h.keys.CopyDetail.Help().Key, h.keys.CopyDetail.Help().Desc},
		{h.keys.FullScreen.Help().Key, h.keys.FullScreen.Help().Desc},
